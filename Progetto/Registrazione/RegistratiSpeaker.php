...
<head>
    ...
    <title>Registrazione Speaker</title>
</head>
...
<p><br><a href="../Homepage.php"> TORNA ALLA HOMEPAGE </a></p>
    ...
    // Evento invio form Registrazione
    if (isset($_POST["submit"])) {
        return registrazioneUtente();
    }


    // Metodo per registrare uno Speaker alla piattaforma
    ...
    function registrazioneSpeaker()
    {
        ...
        try {
            ...
            $nomeUniversità = $_POST["nomeUniversita"];


            $uploadFile = $_FILES['upload']['tmp_name'];
            $foto = (file_get_contents($uploadFile));

            $sql2 = 'CALL registrazioneSpeaker(:username,:foto,:curriculum,:nomeDipartimento,:nomeUniversita)';
            $statement2 = $pdo->prepare($sql2);
            $statement2->bindParam(':username', $username, PDO::PARAM_STR);
            $statement2->bindParam(':foto', $foto, PDO::PARAM_LOB);
            $statement2->bindParam(':curriculum', $curriculum, PDO::PARAM_STR);
            $statement2->bindParam(':nomeDipartimento', $nomeDipartimento, PDO::PARAM_STR);
            $statement2->bindParam(':nomeUniversita', $nomeUniversità, PDO::PARAM_STR);
            $statement2->execute();
