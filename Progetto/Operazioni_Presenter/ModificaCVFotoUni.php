...
<body>
    ...
    <form id="form1" action="" method="post" style="margin: 150px;" enctype='multipart/form-data'>
        ...
        $pdo = ConnDb::connToDb();
        try {
            $selectInfoPresenter = "SELECT Curriculum,NomeDipartimento,NomeUniversita,Foto FROM PRESENTER WHERE UsernamePresenter='$nome'";
            $arrayInfo = $pdo->query($selectInfoPresenter);
    ...
    function InserisciInfoConFoto()
    {
        ...
        try {
            ...
            $sql1 = 'CALL modificaCVFotoUniPresenter(:nome, :foto, :curriculum, :nomeDipartimento, :nomeUni )';
            ...
            echo '<script language="javascript">';
            echo 'alert("ERRORE: impossibile aggiornare i dati")';
            echo '</script>';
            exit();
        }
    }
    ...
    function InserisciInfoSenzaFoto()
    {
        ...
        try {
            ...
            $sql1 = 'CALL modificaCVUniPresenter(:nome,:curriculum, :nomeDipartimento, :nomeUni )';
            $statement1 = $pdo->prepare($sql1);
            $statement1->bindParam(':curriculum', $curriculum, PDO::PARAM_STR);
            $statement1->bindParam(':nomeDipartimento', $nomeDip, PDO::PARAM_STR);
            $statement1->bindParam(':nomeUni', $nomeUni, PDO::PARAM_STR);
